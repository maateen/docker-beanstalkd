...
buildNpush() {
    ...
    docker build -t $repo_name:$beanstalkd_version-alpine-$alpine_version --build-arg ALPINE_VERSION=$alpine_version --build-arg BEANSTALKD_VERSION=$beanstalkd_version .
